...
# Alpha V0.3 Fundamental &
def BTCStrategy(priceMapping, fundamentalDf, next_state: dict):
    ...
    btc_ema_2 = ta.ema(df["BTC-USDT"], length=2 * 24 * 60).to_numpy()
    btc_ema_5 = ta.ema(df["BTC-USDT"], length=5 * 24 * 60).to_numpy()
    ...
    btc_ema_14 = ta.ema(df["BTC-USDT"], length=14 * 24 * 60).to_numpy()
    btc_ema_28 = ta.ema(df["BTC-USDT"], length=28 * 24 * 60).to_numpy()
    ...
    if decision_payload is None:
        errorLogger = Logger("BTC-USDT/ErrCommodore")
